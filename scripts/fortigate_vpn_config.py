"""
Fortigate IPSec VPN 配置脚本
=============================
通过 FortiOS REST API 配置 IPSec VPN 隧道。

使用前需在 Fortigate 上启用 API Token：
...
class FortigateVPNConfigurator:
    """Fortigate IPSec VPN 配置器，使用 FortiOS REST API。"""

    def __init__(self, host: str, api_token: str, verify_ssl: bool = False):
        """
        Args:
            host: Fortigate 管理IP（如 192.168.1.99）
    ...
    def _request(self, method: str, endpoint: str, data: Dict = None) -> Dict[str, Any]:
        ...
        if result.get("http_status") != 200 and result.get("http_status") != 201:
            raise Exception(f"API请求失败 [{endpoint}]: {json.dumps(result, indent=2)}")
        return result
    ...
    def apply_full_config(self, params: Dict) -> Dict:
        ...
        Args:
            params: 配置参数字典（见 vpn_ipsec_plan.md 中的 JSON 模板）
